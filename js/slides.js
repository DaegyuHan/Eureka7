...
요소를 가져오는 이유는 html 문서에 있는 id를 통해 일치하는 요소를 가져와 코드로 조작하기 위해서입니다. 위 코드에서는 각각 슬라이더 , 이전 버든 , 다음 버튼입니다.

(id가 아닌 class일 경우 document.getElementsByClassName을 사용합니다.)
(document.querySelector("#Id") 또는 document.querySelector(".class")를 사용하여 아이디와 클래스 둘다 사용도 가능합니다.)
(주의 : querySelector로 가져온 클래스는 첫번째 요소만 적용됩니다. + 반대로 오히려 이걸 응용도 가능 )
(document.querySelectorAll("#id")를 사용한다면 아이디도 클래스처럼 같은 아이디가 있다면 전체 적용이 가능합니다.)
(querySelectorAll(".class")와 querySelectorAll("#id, .class, #id2, .class2")도 가능합니다.)
(querySelector에 쿼리라는 단어가 처음에는 햇갈릴수 있지만 JavaScript 기본 메서드여서 jQuery라이브러리는 필요없습니다.)
(getElementBy가 성능은 더 뛰어나지만 성능이 크게 중요하지 않은 경우에는 편의성측면에서는 개인적으로 query가 좋다고 생각합니다.)
...
위 코드는 document.getElementById를 통해 가져온 다음 버튼이 addEventListener('click', ()을 이용하여 클릭되었을 경우,
if 문을 통하여 currentIndex < 4 를 통해 다음 버튼을 클릭했을 경우에 해당 인덱스가 마지막이 아닌 경우에는 currentIndex++ 를 통하여 인덱스를 
증가시키고 slider.style.transform = `translateX(-${currentIndex * 100}%)에서 슬라이더의 스타일을 조작을 통해
currentIndex의 인덱스를 통해 해당 슬라이드 인덱스에 맞는 translateX를 조작하여 인덱스의 슬라이드를 이동시켜 다음 슬라이드로 넘어갑니다.
(currentIndex < 4 인 이유는 현재 슬라이드가 5개이기 떄문에 인덱스4가 마지막이기 때문입니다.)
...
1일경우 -1x100을 통하여 마찬가지로 translateX를 -100% 로 조작하여 왼쪽으로 100% 만큼 이동하도록 구현하여 두 번째 슬라이드로 움직이게 조작합니다.) 
ㄴ-100% 인대 오른쪽이 아닌 왼쪽으로 100% 만큼 이동한다는 뜻은 인덱스가 1일 경우 2번쨰 슬라이드임을 뜻하므로,
ㄴ-100%로 조작하게 돤다면 0%인 0번인덱스가 -100%가 아닌 100%가 되어 왼쪽으로 밀리게되어 2번째 슬라이드가 화면에 출력되기 떄문입니다.
...
즉,updateButtons() 함수를 호출하여 해당 함수가 사이트 이용자가 첫번쨰와 마지막 슬라이드에 있을 경우 각각 이전 과 다음 버튼을 숨기기 위해서 작성했던 함수이기때문에
...
이 상태라면 인덱스가 증가하고 슬라이드를 이동시키기전에 첫번째 혹은 마지막 슬라이드인지 확인해버리기 때문에 버튼 숨김이 제대로 작동하지 않는 버그가 발생합니다.
                                    ㄴ다음 슬라이드로 넘어간 후가 아닌 넘어가기 전 상태
...
즉, 인덱스가 증가한 상태에서 -${currentIndex * 100}% 할 경우  translateX의 -%가 증가해 오른쪽으로 이동하게 되고 감소한 상태 일 경우 -%가 감소하여 왼쪽으로 이동합니다.
...
(e.key === 'ArrowRight' 뒤에 논리 연산자인 &&를 사용했기에 AND 또는 그리고 라고 생각하면 됩니다. &&로 인해 키만 눌렀을 경우가 아닌 currentIndex < 4 까지 조건을 만족해야 합니다.)
...
사이트 이용자가 접속 시 외부의 스타일시트와 스크립트가 작동하기 이전에 updateButtons 함수를 실행하여
...
(외부는 html문서 안에 함께 넣은 css 나 스크립트가 아닌 것을 뜻합니다.)
