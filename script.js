...
Html의 슬라이더(화면)를 이전 버튼과 다음 버튼 및 키보드 좌 우 방향키를 통해 조작가능한 슬라이드 기능을 구현하는 자바 스크립트입니다.
...
(요소를 가져오는 이뉴는 html 문서에 있는 id를 통해 일치하는 요소를 가져와 코드로 조작하기 위해서입니다. 위 코드에서는 각각 슬라이더 , 이전 버든 , 뒤로 버튼입니다.)
...
(0번부터 시작떄문에 인덱스0번이 1번 슬라이더 인덱스4번이 5번슬라이더입니다.)
